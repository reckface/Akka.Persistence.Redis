...
namespace Akka.Persistence.Redis.Query.Stages
{
    internal class EventsByTagSource : GraphStage<SourceShape<EventEnvelope>>
    {
        ...
        public EventsByTagSource(ConnectionMultiplexer redis, int database, Config config, string tag, long offset,
        ...
        {
            return new EventsByTagLogic(this);
        }
        ...
        private class EventsByTagLogic : GraphStageLogic
        {
            ...
            public EventsByTagLogic(EventsByTagSource parent) : base(parent.Shape)
            {
            ...
            public override void PreStart()
            {
                ...
                if (_live)
                {
                    ...
                    _subscription.Subscribe(_journalHelper.GetTagsChannel(),
                ...
                else
                {
                    ...
                    _redis.GetDatabase(_database).ListLengthAsync(_journalHelper.GetTagKey(_tag, _isClustered))
                        .ContinueWith(task =>
                        {
                    ...
                    case State.Idle:
                        if (_buffer.Count == 0)
                        {
                            ...
                            var refs = database.ListRange(
                                    _journalHelper.GetTagKey(_tag, _isClustered),
                                    _currentOffset,
                                    Math.Min(_maxOffset, _currentOffset + _max - 1))
                                .Select(tag => tag.Deserialize())
                                .ToArray();
